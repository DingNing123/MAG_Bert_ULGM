...
class MMDataset(Dataset):
    ...
    def __init_mosi(self):
        with open(self.args.dataPath, 'rb') as f:
            data = pickle.load(f)
        # 0318 ding ning plan to update train_samples parameter here
        # and validate parameter copy or 引用 指针 
        ...
        if self.mode == 'train':
            self.args.train_samples = self.labels['M'].shape[0]
        # 更新后 别名 外部的 args 也跟着变了 指向了同样的内存 
