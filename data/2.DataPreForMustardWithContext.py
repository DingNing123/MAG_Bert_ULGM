...
class MDataPre():
    ...
    def run(self):
        if args.mode == 'debug':
            ...
            output_path = os.path.join(self.working_dir, 'Self-MM-Processed/featuresContext.pkl')
        # load last point
        ...
        if args.mode == 'debug':
            ...
            args.df = pd.read_csv(os.path.join(self.working_dir, 'labelContext.csv'), dtype={'clip_id': str, 'video_id': str, 'text': str})
...
def gen_label_csv(args):
    ...
    '''
    # pdb.set_trace()
    label_csv = os.path.join(args.working_dir, 'labelContext.csv')
