...
class MAG_BERT():
    ...
    def do_train(self, model, dataloader):
        ...
        model_params_other = [p for n, p in list(model.Model.named_parameters()) if 'text_model' not in n and \
                                'audio_model' not in n and 'video_model' not in n and 'bert' not in n]
        ...
        # 使用Adam优化器对不同的参数以不同的学习率学习
        # 有的参数在更新的过程中衰减,有的不衰减
        # 通过这种方式加速收敛,而且收敛到最合适的一个点上.
        ...
        # 0320 ding ning see KeyEval = 'Acc' 
        ...
        while True: 
            ...
            for m in self.args.tasks:
                ...
                if m == 'M' :
                    wandb.log({
                            'train_acc': train_results['precision'],
            ...
            wandb.log({
                    'test_acc': test_results['precision'],
                    'test_recall': test_results['recall'],
                    'test_f_score': test_results['f1_score'],
                    'test_loss': test_results['Loss'],
                })

            # 0321 print a new line to formate the output 
            # print('新的一轮已经验证')
            # print()
            logger.info('新的一轮已经验证')
            logger.info('')
            ...
            if epochs - best_epoch >= self.args.early_stop:
                ...
                # I think here should return best_epoch to check the model fitting the data well or not 
                return best_epoch
                # return   0404 change return
    ...
    def weighted_loss(self, y_pred, y_true, indexes=None, mode='fusion'):
        ...
        if mode == 'fusion':
            ...
            # weighted = torch.relu(torch.abs(self.label_map[mode][indexes] - self.label_map['fusion'][indexes]))
