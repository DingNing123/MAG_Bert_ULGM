...
class MetricsTop():
    def __init__(self, train_mode):
        if train_mode == "regression":
            self.metrics_dict = {
                ...
                'MUSTARD': self.__eval_mustard_regression
            }
        else:
            self.metrics_dict = {
    ...
    def __eval_mosei_regression(self, y_pred, y_true, exclude_zero=False):
        ...
        # 0508
        # 参考mmsa项目的源代码 为了获得recall的计算方法
        result_dict = classification_report( binary_truth,binary_preds, digits=3, output_dict=True)
        recall = result_dict["weighted avg"]["recall"]
        precision = result_dict["weighted avg"]["precision"]
        f1_score_cls_rpt = result_dict["weighted avg"]["f1-score"]

        eval_results = {
            "precision": precision,
            "recall" : recall,
            "f1_score": f1_score_cls_rpt,
            # "Mult_acc_2": mult_a2,
            # "Mult_acc_3": mult_a3,
            # "Mult_acc_5": mult_a5,
            # "F1_score": f_score,
...
#       }
    ...
    def __eval_mustard_regression(self, y_pred, y_true):
    ...
    def __eval_sims_regression(self, y_pred, y_true):
        ...
        # 参考mmsa项目的源代码 为了获得recall的计算方法
        result_dict = classification_report(test_truth_a2, test_preds_a2, digits=3, output_dict=True)
        recall = result_dict["weighted avg"]["recall"]
        precision = result_dict["weighted avg"]["precision"]
        f1_score_cls_rpt = result_dict["weighted avg"]["f1-score"]
